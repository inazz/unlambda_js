...
page.OutputPanel.prototype.addCompileErrorMessage_ = function(err, err_pos) {
  var span = this.dom_helper.createElement('span');
  this.dom_helper.appendChild(this.output_block, span);
...
page.OutputPanel.prototype.maybeAddCodeSnippet_ = function(code, err, err_pos) {
  var char_err = true;
  switch (err) {
  case unlambda.parser.ERROR.UNEXPECTED_CHARACTER:
    break;
  ...
  this.dom_helper.appendChild(this.output_block, code_node);
  if (err_pos != 0) {
    var text_node = this.dom_helper.createTextNode();
